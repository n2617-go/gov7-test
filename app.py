...
def fetch_and_analyze(stock_id):
    ...
    # --- 關鍵修正：強制轉換為純 Series 以符合 ta 套件需求 ---
    ...
    if last['MA5'] > last['MA10'] > last['MA20']:
        results.append("✅ 均線：多頭排列"); score += 1
    if last['K'] > last['D'] and last['K'] > 20:
        results.append("✅ KD：低檔轉強"); score += 1
    if last['MACD_diff'] > 0:
        results.append("✅ MACD：柱狀體正向"); score += 1
    if last['RSI'] > 50:
        results.append("✅ RSI：強勢區"); score += 1
    if last['Close'] > last['BBM']:
        results.append("✅ 布林：站穩中軸"); score += 1
    ...
    if res:
        with st.container(border=True):
            ...
            with col1:
                ...
                st.write(f"**技術等級：{res['grade']}** ({res['score']}/5)")
            with col2:
            ...
            if is_open and abs(res['pct']) >= st.session_state.tg_threshold:
                key = f"alert_{stock['id']}_{datetime.now(tw_tz).strftime('%Y%m%d')}"
                if key not in st.session_state.alert_history:
                    msg = (f"🚨 <b>【台股異動】</b>\n{stock['name']} ({stock['id']})\n"
                           f"價格：{res['price']:.2f} ({res['pct']:+.2f}%)\n"
                           f"等級：{res['grade']}\n符合：{', '.join(res['details'])}")
